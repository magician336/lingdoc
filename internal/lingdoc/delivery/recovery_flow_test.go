...
func TestRecoveryFlowBlockedInputDoesNotInvokeRenderer(t *testing.T) {
	...
	if _, err := service.Start(snapshot.ProjectID, snapshot.ID); !errors.Is(err, ErrExportPreflightBlocked) {
...
func TestRecoveryFlowFailedRenderCanRetryWithoutExposingOldBytes(t *testing.T) {
	...
	exports := NewMemoryExportStore()
	failed := NewExportService(snapshots, exports, frozenRenderer(func(DeliveryInput) ([]byte, error) {
		return nil, errors.New("temporary renderer outage")
	}))
	first, err := failed.Start(snapshot.ProjectID, snapshot.ID)
	if err != nil || first.Status != ExportFailed {
		t.Fatalf("failed export = %+v, %v", first, err)
	}
	if _, err := failed.Download(snapshot.ProjectID, first.ID); !errors.Is(err, ErrExportUnavailable) {
		t.Fatalf("failed artifact became downloadable: %v", err)
	}

	// A retry is a new explicit export, not a mutation of a failed file.
	failed.renderer = frozenRenderer(func(DeliveryInput) ([]byte, error) {
		return []byte("PK\\x03\\x04recovered docx"), nil
	})
	second, err := failed.Start(snapshot.ProjectID, snapshot.ID)
	if err != nil || second.Status != ExportVerified || second.ID == first.ID {
		t.Fatalf("retried export = %+v, %v", second, err)
	}
	if _, err := failed.Download(snapshot.ProjectID, second.ID); err != nil {
